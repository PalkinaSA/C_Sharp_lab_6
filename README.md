...
3. Создаём статический класс Funs для методов meowCare(), принимающих множество мяукающих объектов. Метод meowCare() может получить один или несколько мяукающих объектов, чтобы затем дать им поочереди помяукать. Также перегуружаем этот метод с использованием раз мяуканий. 
...
6. Затем создаём специальный класс MeowCounter с интерфейсом Meowable, чтобы можно было считать количество мяуканий мяукающих объектов. Он принимает в себя мяуающий объект и количество мяуканий. Создаём конструктор с параметром в виде мяукающего объекта. Определяем методы meow без параметров и с параметром times так, чтобы они считали количество мяуканий. Создаём метод GetCorrectTimesForm() для получения корректной формы слова "разы". Переопредяем метод ToString().
...
<img width="689" height="253" alt="Коты" src="https://github.com/user-attachments/assets/5bff2f37-51c4-4460-9d4f-aecbc70aa72e" />
